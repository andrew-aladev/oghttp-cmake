  ...
  size_t*        next_state_by_last_symbols_length_ptr)
{
  ...
  if (next_state_by_last_symbols == NULL) {
    PRINTF_ERROR(
      "failed to allocate memory for next state by last symbols, size: %zu", next_state_by_last_symbols_size);
    return 1;
  }
    ...
    for (size_t jndex = 0; jndex < strlen(prefix_constant); jndex++) {
      ...
      if (!is_constant_state_exist) {
        if (global_state >= max_state) {
          PRINTF_ERROR(
            "global state should be less than max state, global state: %zu, max state: %zu", global_state, max_state);
  ...
  size_t         max_state)
{
  ...
  int result = init_next_state_by_last_symbols(
    ...
    &next_state_by_last_symbols_length);
